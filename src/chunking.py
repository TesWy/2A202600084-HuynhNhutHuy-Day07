...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        # Base case: text đủ ngắn
        if len(current_text) <= self.chunk_size:
            return [current_text] if current_text.strip() else []
        
        # Hết separator mà vẫn dài → force split by character
        ...
        # Separator cuối (empty string) = force split
        if sep == "":
            return [
                current_text[i:i + self.chunk_size] 
                for i in range(0, len(current_text), self.chunk_size)
        ...
        # Nếu không tách được (không có separator này), thử separator tiếp theo
        if len(parts) == 1:
            return self._split(current_text, remaining_separators[1:])
        
        # Tách được → xử lý từng phần đệ quy
        results = []
        for part in parts:
            if part.strip():  # Bỏ qua empty strings
                # Đệ quy với toàn bộ separator list (thử lại từ đầu)
